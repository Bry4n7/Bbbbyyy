// --- 1. FULL 48 TEAM DATA & PLAYERS (Newly Updated - Realistic 2026 Call-ups) ---
const worldCupData = {
  "Group A": [
...
function renderGroups() {
  ...
  for (const [groupName, teams] of Object.entries(worldCupData)) {
    let teamsHtml = teams.map(t => `
      <div class="team-entry" onclick="selectTeam('${t.team}')">
        ...
        container.innerHTML += `
          <div class="match-row">
            ...
            <div style="text-align: right;">
              ...
              <button class="sim-btn" onclick="simulateMatch(this, ${tA.strength}, ${tB.strength})">⚽ SIM MATCH</button>
            </div>
          </div>`;
    }
    dateCounter++; 
...
const allTriviaQuestions = [
  ...
  { q: "Which nation won the 2006 World Cup?", options: ["France", "Brazil", "Germany", "Italy"], ans: 3 },
  { q: "What was the name of the official ball used in the 2010 South Africa World Cup?", options: ["Brazuca", "Jabulani", "Teamgeist", "Telstar"], ans: 1 },
  { q: "Who is the only manager to win two consecutive World Cups?", options: ["Vittorio Pozzo", "Mario Zagallo", "Didier Deschamps", "Franz Beckenbauer"], ans: 0 },
  { q: "Which team scored exactly 7 goals against Brazil in the 2014 semi-final?", options: ["Netherlands", "Argentina", "Germany", "Colombia"], ans: 2 },
  { q: "Who holds the record for the most clean sheets in World Cup history?", options: ["Iker Casillas & Gianluigi Buffon", "Fabien Barthez & Peter Shilton", "Manuel Neuer & Hugo Lloris", "Oliver Kahn & Dino Zoff"], ans: 1 }
];
...
function loadTriviaQuestion() {
  ...
  if(currentTriviaIndex >= sessionTrivia.length) {
    container.innerHTML = `
      ...
      <button class="trivia-btn" style="margin-top:20px; border-color:var(--wc-cyan);" onclick="startNewTriviaSession()">PLAY NEW ROUND</button>
    `;
    return;
  }
  ...
  let optionsHtml = qData.options.map((opt, idx) => 
    `<button class="trivia-btn" onclick="checkTriviaAnswer(${idx}, ${qData.ans})">${opt}</button>`
  ).join('');
...
// --- 8. GAME 6: DRIBBLE DASH (WITH REAL DEFENDER IMAGES) ---
...
// Load the real picture of a defender slide tackling
...
function updateDribble() {
  ...
  if(!dDead) {
    ...
    // Spawn Defender Enemies
    if(Math.random() < 0.05 * dSpeedMultiplier) {
      // Made them slightly wider and taller so the photo looks like an actual person sliding
    ...
    for(let i = dEnemies.length - 1; i >= 0; i--) {
      ...
      // Draw actual sliding defender image onto the canvas block
